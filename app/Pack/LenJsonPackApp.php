...
class LenJsonPackApp implements IPack
{
    ...
    {
        secho('app-encode',$buffer);
        $total_length = $this->package_length_type_length + strlen($buffer) - $this->package_body_offset;
        secho('app-encode---$total_length',$total_length);
        $res = pack($this->package_length_type, $total_length) . $buffer;
        secho('app-encode---$res',$res);
        return $res;
    }
